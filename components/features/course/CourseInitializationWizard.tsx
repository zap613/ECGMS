...
export function CourseInitializationWizard({ isOpen, onClose, onComplete }: WizardProps) {
  ...
  React.useEffect(() => {
    if (step === 2) {
      const studentsPerClass = Math.floor(totalStudents / classCount)
      const newCourses = Array.from({ length: classCount }, (_, i) => ({
  ...
  const handleNextStep = () => {
    if (!semesterName || totalStudents <= 0) {
  ...
  const handleFinalize = () => {
    ...
    alert(`Đã khởi tạo thành công ${classCount} lớp với tổng số ${emptyGroupsToCreate * classCount} nhóm trống!`)
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input id="totalStudents" type="number" value={totalStudents} onChange={e => setTotalStudents(parseInt(e.target.value) || 0)} />
              </div>
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-6 py-4">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Input id="classCount" type="number" min="1" value={classCount} onChange={e => setClassCount(parseInt(e.target.value) || 1)} className="col-span-1" />
              <p className="col-span-2 text-sm text-muted-foreground">
                ~{Math.floor(totalStudents / classCount)} sinh viên / lớp
              </p>
            </div>
